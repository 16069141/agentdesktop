"""内置工具实现（filesystem / shell / knowledge / code）。

规格书 §9.3：
- filesystem：受限根目录 + 审计
- shell：白名单 + 需确认 + 审计
- knowledge：RAG 检索（ACL 鉴权）
...
class KnowledgeTool:
    ...
    def __init__(self, rag_pipeline=None, audit_log: list | None = None):
    ...
    async def __call__(self, args: dict) -> str:
        ...
        if self.rag is None:
            msg = "RAG Pipeline 未初始化（chromadb 未安装），请安装后重试。"
            self._audit("knowledge", query, allowed=False, reason="rag_not_available")
            return msg

        try:
            results = await self.rag.search(query, limit=top_k)
            if not results:
                msg = f"知识库中未找到与「{query}」相关的文档片段"
            else:
                parts = []
                for i, r in enumerate(results, 1):
                    ...
                    score = r.get("score", 0)
                    parts.append(f"[{i}] {source}（相关度 {score:.3f}）\n    {snippet}")
                msg = "\n\n".join(parts)
            self._audit("knowledge", query, allowed=True)
            return msg
        except Exception as exc:
            msg = f"RAG 检索失败: {exc}"
            logger.error(f"[knowledge] 检索异常: {exc}")
            self._audit("knowledge", query, allowed=False, reason=str(exc))
            return msg

    def _audit(self, tool: str, query: str, allowed: bool, reason: str = ""):
        self.audit_log.append({"tool": tool, "query": query, "allowed": allowed, "reason": reason})
        logger.info(f"[audit] knowledge search '{query[:30]}' allowed={allowed}")
